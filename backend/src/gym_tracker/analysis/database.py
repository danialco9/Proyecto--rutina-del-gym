...
def load_user_log(session: Session, user_id: int) -> TrainingLog:
    """Everything the analysis needs for one user: visible exercises, routines, workouts and measurements.

    A workout's ``date`` is the calendar day of ``started_at`` in UTC.
    """
    exercises = select(
    ...
    workout_rows = [(row[0], row[1].astimezone(UTC).date(), *row[2:]) for row in _rows(session, workouts)]
    return TrainingLog(
